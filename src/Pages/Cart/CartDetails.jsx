...
import { getProductsOfCart, removeProductFromCart } from "../../Redux/Slice/CartSlice";
...
function CartDetails(){
    ...
    async function fetchCartDetails(){
        const response = await dispatch(getProductsOfCart())
                        ...
                        <div className="flex-1 w-full min-w-0 md:order-2 md:max-w-md">
                          <p className="text-base font-medium text-gray-900 hover:underline">
                            <Link to={`/product/${item?._id}`}>
                              {`${item?.product?.name}, Category: ${item?.product?.category}`}
                            </Link>
                          </p>
